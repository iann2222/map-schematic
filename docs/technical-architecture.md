...
- 開發模式與封裝版本共用已記住的資料根目錄，避免同一台電腦保存兩份相同資料包。
- 第一次從已有資料包的開發版啟動時，會以 repo 根目錄作為共用位置；尚無資料包時，預設使用 `%LOCALAPPDATA%\map-schematic`，避免寫入唯讀安裝目錄。
- 可使用 `MAP_SCHEMATIC_ROOT` 暫時覆寫資料根目錄，供開發、測試或可攜式部署使用。
...
- 若 `pack-release.json` 指向新版本，啟動時繼續使用 `active.json` 指向的有效舊版；使用者可在偏好設定查看版本並主動下載更新。
- 已安裝資料包損壞且無法恢復時，先詢問使用者；只有取得同意才連網重新下載。
- 解壓前會拒絕路徑跳脫、符號連結與異常大的 ZIP 內容；下載、checksum、解壓或驗證失敗會回報錯誤並清理暫存檔，不把不完整資料包當成可用版本。
...
6. App 執行期間只以本機資料檢查資料包狀態；使用者在偏好設定主動確認更新後，才連線下載目標版本並重新載入本機資料。
...
- 專案內容異動後會標記為尚未儲存；關閉視窗時可選擇儲存、不儲存或取消，載入其他專案前也會先確認。
- schema 合法但目前 renderer 尚不能編輯的幾何物件不會被靜默丟棄；載入時會提示，後續儲存仍原樣保留。
...
- 偏好設定會顯示目前版本、目標版本與修復狀態；已有有效舊版時，只有使用者主動按下更新並確認後才下載
- 不在一般編輯流程中自動連網檢查或下載
