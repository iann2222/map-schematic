...
- 目前 schema 版本為 `0.1`；逐版 migration 與相容矩陣是下一階段工作。
...
- 目前只接受 schema `0.1`
- 下一階段將加入逐版 migration、原子儲存、上一份有效檔案備份與使用者確認後恢復
...
目前自動化測試涵蓋 `.mapproj` validator、序列化、儲存與載入。下一階段將擴充 schema migration、原子儲存、備份恢復、資料包切換與匯出測試。
